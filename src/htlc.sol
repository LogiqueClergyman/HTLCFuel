...
contract HTLC is EIP712 {
     ...
     */
    modifier safeParams(address redeemer, uint256 timelock, uint256 amount) {
    ...
    function initiate(address redeemer, uint256 timelock, uint256 amount, bytes32 secretHash)
        external
        safeParams(redeemer, timelock, amount)
    {
    ...
    function redeem(bytes32 orderID, bytes calldata secret) external {
        ...
        require(sha256(abi.encode(block.chainid, secretHash, order.initiator)) == orderID, "HTLC: incorrect secret");
     ...
     * @param   initiator_   The address of the refunder of the atomic swap
    ...
    function _initiate(
        ...
        bytes32 orderID = sha256(abi.encode(block.chainid, secretHash_, initiator_));
        ...
        token.safeTransferFrom(funder_, address(this), orders[orderID].amount);
    }
    ...
    function instantRefund(bytes32 orderID, bytes calldata signature) external {
        address redeemer = instantRefundDigest(orderID).recover(signature);
        Order storage order = orders[orderID];
 ...
 * @notice  A proxy contract that allows users to have unique deposit addresses for HTLC swaps
...
contract UniqueDepositAddress is Initializable {
     ...
     * @dev     Should be called after the user deposits funds to the address
     * @param   initiator  Address that will be able to refund the swap
     * @param   redeemer   Address that can redeem the swap with the secret
     * @param   timelock   Number of blocks before initiator can refund
     * @param   secretHash Hash of the secret required to redeem
     */
    function initialize(address initiator, address redeemer, uint256 timelock, bytes32 secretHash) public initializer {
        IERC20 token = htlc.token();
        uint256 amount = token.balanceOf(address(this));
        token.approve(address(htlc), amount);
        htlc.initiateOnBehalf(initiator, redeemer, timelock, amount, secretHash);
        owner = initiator;
    }

    /**
     * @notice  Allows the owner to recover any tokens accidentally sent to this contract
     * @dev     Always sends the funds to the owner (initiator) of the contract
     * @param   _token  The ERC20 token to recover
     */
    function recover(IERC20 _token) public {
